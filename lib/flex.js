...
function FlexImage(config){
	...
	if([2,4].indexOf(imageConfig.length)!=-1){
		this.x = this.imageConfig[0];
		this.y = this.imageConfig[1];
	}
	if(imageConfig.length==4){
		this.width = this.imageConfig[2];
		this.height = this.imageConfig[3];
	}else if(imageConfig.length==8){
		this.x = this.imageConfig[4];
		this.y = this.imageConfig[5];
...
FlexImage.prototype.renderImage = function(image,renderConfig){
	...
	if(renderConfig.length==2){
		app.context.drawImage(image,this.stageX,this.stageY);
		this.width = image.width;
		this.height = image.height;
	}else if(renderConfig.length==4){
		app.context.drawImage(image,this.stageX,this.stageY,this.width,this.height);
	}else{
		app.context.drawImage(image,renderConfig[0],renderConfig[1],renderConfig[2],renderConfig[3],this.stageX,this.stageY,renderConfig[6],renderConfig[7]);
	}
};
...
function HGroup(config){
	...
	this.paddings = config.paddings || [0,0,0,0];
}

HGroup.prototype = new DisplayObject();
