...
FlexImage.prototype.load = function(){
	var self = this;
	var decorateConfig = self.imageConfig.concat();//获取config的副本
	var commandStr;
	var argLen = decorateConfig.length+'';
	if(!this.image){
		this.image = new Image();
		this.image.src = this.src;
		this.image.onload = function(){
			commandStr = "app.context.drawImage(this,";
			for(var i=0;i<decorateConfig.length;i++){
				if(i==decorateConfig.length-1){
					commandStr += "decorateConfig["+i+"])";
				}else{
					commandStr += "decorateConfig["+i+"],";
				}
			}
		...
		commandStr = "app.context.drawImage(this.image,";
			for(var i=0;i<decorateConfig.length;i++){
				if(i==decorateConfig.length-1){
					commandStr += "decorateConfig["+i+"])";
				}else{
	...
	switch(argLen){
		case '2':
			...
			decorateConfig[0] = this.stageX = this.parent.stageX + this.imageConfig[0];
			decorateConfig[1] = this.stageY = this.parent.stageY + this.imageConfig[1];
		...
		case '8':
			...
			decorateConfig[4] = this.stageX = this.parent.stageX + this.imageConfig[4];
			decorateConfig[5] = this.stageY = this.parent.stageY + this.imageConfig[5];
