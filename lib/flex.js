...
function DisplayObject(config){
	...
	this._children = [];
	this._numChildren = 0;
	this.name = config.name || "DisplayObject";
	this.stageX = 0;
	this.stageY = 0;
	...
	addChildren:function(){
		...
		for(var i=0;i<arguments.length;i++){
			...
			if(child instanceof DisplayObject){
				...
				trace(child+"不是DisplayObject的实例","warn");
			}
...
FlexImage.prototype.renderImage = function(image,renderConfig){
	var decorateConfig = renderConfig.concat();//用完之后置空 然后进行垃圾回收
	if([2,4].indexOf(renderConfig.length)!=-1){
		this.x = renderConfig[0];
		this.y = renderConfig[1];
		decorateConfig[0] = this.stageX = this.parent.stageX + renderConfig[0];
		decorateConfig[1] = this.stageY = this.parent.stageY + renderConfig[1];
	}
	if(decorateConfig.length==2){
		app.context.drawImage(image,decorateConfig[0],decorateConfig[1]);
		this.width = image.width;
		this.height = image.height;
	}else if(decorateConfig.length==4){
		app.context.drawImage(image,decorateConfig[0],decorateConfig[1],decorateConfig[2],decorateConfig[3]);
		this.width = renderConfig[2];
		this.height = renderConfig[3];
	}else{
		this.x = renderConfig[4];
		this.y = renderConfig[5];
		decorateConfig[4] = this.stageX = this.parent.stageX + renderConfig[4];
		decorateConfig[5] = this.stageY = this.parent.stageY + renderConfig[5];
		app.context.drawImage(image,decorateConfig[0],decorateConfig[1],decorateConfig[2],decorateConfig[3],decorateConfig[4],decorateConfig[5],decorateConfig[6],decorateConfig[7]);
		this.width = renderConfig[6];
		this.height = renderConfig[7];
	}
	decorateConfig = null;
};
FlexImage.prototype.load = function(){
	var self = this;//保存this引用  方便在onload函数中使用
	if(!this.image){
		this.image = new Image();
	}
	if(this.image.src.indexOf(this.src)==-1){//如果是第一次加载图片
		this.image.src = this.src;
		this.image.onload = function(){
			self.renderImage(this,self.imageConfig);
		}
	}else{
		self.renderImage(self.image,self.imageConfig);
	}
};
/**
 * 渲染方法
